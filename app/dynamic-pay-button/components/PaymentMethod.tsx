...
export const PaymentMethod = ({ checked, variant }: PaymentMethodProps) => {
  return (
    <div>
      ...
      <label
        className={cn(
          'u-flex u-h-12 u-items-center u-justify-between u-rounded-lg u-border u-px-3 u-transition-colors active:u-scale-[0.98] sm:u-px-4',
      ...
      >
        {variant === 'googlePay' && (
          <>
            <p className={cn('u-truncate u-text-xs sm:u-text-sm')}>
              Google Pay
            </p>
        ...
        {variant === 'applePay' && (
          <>
            <p className={cn('u-truncate u-text-xs sm:u-text-sm')}>Apple Pay</p>
            <ApplePay className={cn('u-relative u-top-[2px] u-h-5')} />
          </>
        )}
        {variant === 'paypal' && (
          <>
            <p className={cn('u-truncate u-text-xs sm:u-text-sm')}>PayPal</p>
            <Paypal className={cn('u-size-4')} />
          </>
        )}
        {variant === 'cashApp' && (
          <>
            <p className={cn('u-truncate u-text-xs sm:u-text-sm')}>Cash App</p>
            <CashApp className={cn('u-size-4')} />
          </>
        )}
        {variant === 'klarna' && (
          <>
            <p className={cn('u-truncate u-text-xs sm:u-text-sm')}>Klarna</p>
