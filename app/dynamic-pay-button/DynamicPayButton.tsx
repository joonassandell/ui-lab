  ...
  return (
    ...
    >
      <div className={cn('flex w-full items-center justify-between px-3 py-3')}>
        ...
        <m.button
          className={cn(
            'flex cursor-default select-none items-center justify-center gap-3 self-start whitespace-nowrap px-3 py-1 focus-visible:outline-none',
          )}
          layout
          onClick={handleOpen}
          transition={TRANS_SPRING}
...
const Card = () => {
  const [dragDirectionX, setDragDirectionX] = useState('right');
  const [dragX, setDragXonly] = useState(false);

  return (
    <div
      className={cn('text-shadow-black/40 h-52 text-white text-shadow')}
    ...
    >
      ...
      >
        <div
          className={cn(
            'relative flex h-full select-none flex-col justify-end gap-3 overflow-hidden rounded-xl p-5 font-cc',
